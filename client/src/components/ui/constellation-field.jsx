...
/**
 * Constellation Field — animated drifting-node canvas background, isolated
 * inside a sandboxed iframe so the full source page (canvas + surrounding
 * markup) can be reused verbatim while only the canvas is shown.
 *
 * Plain JavaScript/JSX only — no TypeScript types, interfaces, or generics.
 *
 * Props (all optional):
 *   mode          "dark" | "light" | "auto"
 *   speed         number, live-controlled (default 1)
 *   size          number, particle radius multiplier (default 1)
 *   gap           number, reserved knob passed through to controls (default 2)
 *   length        number, link/connection distance multiplier (default 1)
 *   density       number, node-count multiplier (default 1)
 *   strokeWidth   number, link line width (default 1)
 *   opacity       number, live-controlled overall opacity (default 1)
 *   hue           number, CSS hue-rotate degrees applied to the iframe (default 0)
 *   saturation    number, CSS saturate() applied to the iframe (default 1)
 *   brightness    number, CSS brightness() applied to the iframe (default 1)
 *   className     string
 *   style         object
 */
...
function patchConstellationField(source, { size, length, density, strokeWidth, mode }) {
  let next = source
